...
// Helper to open a file for reading
...
// Helper to close a file descriptor
void closeFd(int Fd) {
#if WASMEDGE_OS_WINDOWS
  _close(Fd);
#else
...
// Helper to flush file descriptor
void flushFd(int Fd) {
#if WASMEDGE_OS_WINDOWS
  _commit(Fd);
#else
...
// Helper to check if a file descriptor is valid
...
TEST(WasiTest, CustomFds) {
  ...
  // Test custom stdout
  {
    ...
    Env.init({}, "test"s, {}, {}, 0, static_cast<uint32_t>(TestFd), 2);
  ...
  // Test custom stdin
  {
    ...
    Env.init({}, "test"s, {}, {}, static_cast<uint32_t>(TestFd), 1, 2);
  ...
  // Test custom stderr
  {
    ...
    Env.init({}, "test"s, {}, {}, 0, 1, static_cast<uint32_t>(TestFd));
  ...
  {
    const std::string_view TestString = "hello from wasi stdout";
    const uint32_t InvalidFd = static_cast<uint32_t>(-1);
